...
class UploadHandler
{
    ...
    protected function ftp_connect() {
        ...
        $_SESSION['conn_id'] = ftp_ssl_connect( $config['ftp_host']) or die("Couldn't connect to $ftp_server"); 

        // login with username and password
        return $login_result = ftp_login($_SESSION['conn_id'], $_SESSION['username'], $_SESSION['password']);
    }

    protected function ftp_list() {
        // close the connection
        return ftp_nlist($_SESSION['conn_id'], ".");
    }
    protected function ftp_file_size($file) {
        return ftp_size($_SESSION['conn_id'], $file);
    }
    protected function ftp_delete($file) {
        return ftp_delete($_SESSION['conn_id'], $file);
    }
    ...
    protected function get_file_objects($iteration_method = 'get_file_object') {
        ...
        if ( $this->options['filesystem'] =="ftp") {
            $this->ftp_connect();
            $files = $this->ftp_list();
            foreach ($files as $each => $filename) {
                // var_dump($filename);
                $file = new stdClass();
                $file->name = $filename;
                $file->size = $this->ftp_file_size($filename);
                $file->url = $this->get_download_url($file->name);
                $this->set_additional_file_properties($file);
                $result[] = $file;
            }
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    move_uploaded_file($uploaded_file, $file_path);
                     ...
                     if ( $this->options['filesystem'] =="ftp") {
                        $this->ftp_connect();
                        ftp_put($_SESSION['conn_id'], $file->name, $file_path,  FTP_BINARY ); 
                        $this->ftp_close();
                        unlink($file_path);
                    } 
                }
            ...
            if ( $this->options['filesystem'] =="ftp") {
                $this->ftp_connect();
                $file_size = $this->ftp_file_size($file->name);
                $this->ftp_close();
            } else {
    ...
    public function delete($print_response = true) {
        ...
        foreach($file_names as $file_name) {
            ...
            if ( $this->options['filesystem'] =="ftp") {
                $this->ftp_connect();
                $success = $file_name[0] !== '.' && $this->ftp_delete($file_name);
                $this->ftp_close();
            } else {
